			...
			if (authority.includes('managed')) {
				...
				return Promise.resolve(new vscode.ManagedResolvedAuthority(async () => {
					...
					await new Promise((res, rej) => {
						remoteSocket.on('data', d => dataEmitter.fire(d as any))
					...
					return {
						...
						send: d => remoteSocket.write(d as any),
			...
			return new Promise<vscode.ResolvedAuthority>((res, _rej) => {
				const proxyServer = net.createServer(proxySocket => {
					...
					proxySocket.on('data', async (data) => {
						await maybeSlowdown();
						remoteReady = remoteSocket.write(data as any);
						if (!remoteReady) {
							proxySocket.pause();
						}
					});
					remoteSocket.on('data', async (data) => {
						await maybeSlowdown();
						localReady = proxySocket.write(data as any);
						if (!localReady) {
							remoteSocket.pause();
						}
					});
